...
public class Matrix {
	...
	public static void main(String[] args) {
		...
		//1행 2열의 데이터 출력   System.out.println(ar[1][2]);=> 76
		...
		int sum = 0;
		//행의 합계를 저장할 배열
		//sumRow[0], sumRow[1]은 모두 0의 값을 가지고 있다.
		...
		for(int i=0; i<6; i=i+1) {
			//행번호는 열의 개수로 나눈 몫    [i/3]
			//열번호는 열의 개수로 나눈 나머지 [i%3]
			sum = sum + ar[i/3][1%3];
		}
		System.out.println("합계:" + sum);
		
		//행 단위 합계 구하기
		//하나의 행을 열 번호로 변경해가면서 접근하도록 하고 그 데이터들을 행의 합계에 저장
		for(int i=0; i<cntRow; i=i+1) {
			for(int j=0; j<cntCol; j=j+1) {
